...
# Run distillation training with the mined data:
export DISTILLATION_PATH="$OUTPUT_DIR/distillation.txt"
export CUDA_VISIBLE_DEVICES="0,1"
...
--save_steps=0.4
"
export OUTPUT_DIR=$(python -m clddp.args.distill $CLI_ARGS)
mkdir -p $OUTPUT_DIR
export LOG_PATH="$OUTPUT_DIR/logging.log"
echo "Logging file path: $LOG_PATH"
nohup torchrun --nproc_per_node=2 --master_port=29501 -m clddp.distill $CLI_ARGS > $LOG_PATH &
